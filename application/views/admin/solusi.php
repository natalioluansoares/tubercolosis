<div class="wrapper">
    <div class="container-fluid">
                                    ...
                                    foreach ($solusi as $rs) : ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="" class="tabledit-delete-button btn btn-sm btn-success btn-animation mb-2" title="Update Data Solusi" data-toggle="modal" data-animation="rubberBand" data-target=".updatesolusi" id="modelsolusi" data-id="<?= $rs['id']; ?>" data-kodesolusi="<?= $rs['kodesolusi']; ?>" data-solusi="<?= $rs['solusi']; ?>" data-idgejala="<?= $rs['id_gejala']; ?>">
            ...
            <div class="modal fade addsolusi" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header" style="background-color: rgb(255, 219, 16); color: white">
                            <h5 class="modal-title" id="exampleModalLabel">INPUT DATA SOLUSI</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="<?= base_url('admin/addsolusi'); ?>" method="POST" style="background-color: rgb(12, 15, 235); color: white">
                            <div class="modal-body">
                                <label for="kodesolusi" style="color: azure;">Kode Solusi</label>
                                ...
                                <label for="namagejala" style="color: azure;">Nama Gejala</label>
                                <div class="input-group">
                                    <select name="namagejala" id="namagejala" class="form-control">
                                        <option value="">Pilih Penyakit</option>
                                        <?php foreach ($gejala as $rs) : ?>
                                            <option value="<?= $rs['id'] ?>"><?= $rs['nama_gejala'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="input-group-append bg-custom b-0"><span class="input-group-text btn btn-info">
                                ...
                                <label for="solusi" style="color: azure;">Solusi</label>
                                <div class="input-group">
                                    <textarea class="form-control" id="solusi" placeholder="Solusi" name="solusi" cols="30" rows="5"></textarea>
                                    <div class="input-group-append bg-custom b-0"><span class="input-group-text btn btn-info">
                                            <i class="fa fa-pencil-square-o"></i></span></div>
                                </div><br>
        ...
        <div class="modal fade updatesolusi" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header" style="background-color: rgb(40, 40, 218); color: white; font:larger">
                        <h5 class="modal-title" id="exampleModalLabel">UPDATE DATA SOLUSI</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="<?= base_url('admin/ubahsolusi'); ?>" method="POST" style="background-color: rgb(255, 219, 16); color: white">
                        <div class="modal-body">
                            ...
                            <div class="input-group">
                                <select name="namagejala" id="gejala" class="form-control">
                                    <?php foreach ($gejala as $rs) : ?>
                                        <option value="<?= $rs['id'] ?>"><?= $rs['nama_gejala'] ?>
                                        </option>
                                    <?php endforeach; ?>
        ...
        <script type="text/javascript">
            $(document).on("click", "#modelsolusi", function() {
                ...
                var Idgejala = $(this).data('idgejala');

                $(".updatesolusi #idsolusi").val(Id);
                $(".updatesolusi #kodesolusi").val(Kodesolusi);
                $(".updatesolusi #solusi").val(Solusi);
                $(".updatesolusi #gejala").val(Idgejala);
            })
            $(document).ready(function(e) {
                $("#form").on("submit", (function(e) {
                    e.preventDefault();
                    $.ajax({
                        url: '<?= base_url('admin/ubahgejala'); ?>',
                        type: 'POST',
                        ...
                        processData: false,
                        success: function() {
                            document.location.href = "<?= base_url('admin/solusi'); ?>";
                        }
                    });
                }));
            })
