<div class="wrapper">
    <div class="container-fluid">
                        ...
                        <div class="tab-pane active p-3" id="kabupaten" role="tabpanel">
                            ...
                            <table id="datatable" class="table table-bordered table-striped">
                                <thead class="table-primary">
                                    <tr>
                                        <th>Company</th>
                                        <th data-priority="1">Konsultasi</th>
                                        <th data-priority="1">Nama Rumah Sakit</th>
                                        <th data-priority="1" class="text-center"><i class="fa fa-cog "></i></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php $no = 1;
                                    foreach ($konsultasi as $rs) : ?>
                                        <tr>
                                            <th> <span class="co-name"><?= $no++; ?></span></th>
                                            <td><?= $rs['nama_konsultasi']; ?></td>
                                            <td><?= $rs['nama_dokter']; ?></td>
                                            <td class="text-center">
                                                <a href="" class="tabledit-delete-button btn btn-sm btn-success btn-animation mb-2" title="Update Data Konsultasi" data-toggle="modal" data-animation="flipInX" data-target=".updatekonsultasi" id="modelkonsultasi" data-id="<?= $rs['id']; ?>" data-namakonsultasi="<?= $rs['nama_konsultasi']; ?>" data-iddokter="<?= $rs['id_dokter']; ?>">
                                                    <i class="ti-pencil"></i></a>
                                                <a href="" class="tabledit-delete-button btn btn-sm btn-danger mb-2" title="Delete Data Konsultasi"><i class="ti-trash"></i></a>
                                                <a href="" class="tabledit-delete-button btn btn-sm btn-warning btn-animation mb-2" data-toggle="modal" data-animation="flipInY" data-target=".addkonsultasi" title="Input Data Konsultasi"><i class="ti-save"></i></a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">INPUT DATA KONSULTASI</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <form action="<?= base_url('dokterperawatbidan/addkonsultasi'); ?>" method="POST">
                            <div class="modal-body">
                                <label for="">Konsultasi</label>
                                <div class="input-group">
                                    <input type="text" class="form-control" id="konsultasi" value="Penyakit Tubercolosis" placeholder="Konsultasi" name="konsultasi" readonly>
                                ...
                                <div class="input-group">
                                    <select name="namadokter" id="namadokter" class="form-control">
                                        ...
                                        <?php foreach ($dokter as $rs) : ?>
                                            <option value="<?= $rs['id_dopebi'] ?>"><?= $rs['nama_dokter'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="input-group-append bg-custom b-0"><span class="input-group-text btn btn-info">
        ...
        <div class="modal fade updatekonsultasi" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">UPDATE DATA KONSULTASI</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form action="<?= base_url('dokterperawatbidan/ubahkonsultasi'); ?>" method="POST">
                        <div class="modal-body">
                            <label for="">Konsultasi</label>
                            <div class="input-group">
                                <input type="hidden" class="form-control" id="idkonsultasi" placeholder="Kode Rumahsakit" name="idkonsultasi">
                                <input type="text" class="form-control" id="konsultasi" placeholder="Konsultasi" name="konsultasi" readonly>
                                <div class="input-group-append bg-custom b-0"><span class="input-group-text btn btn-info">
                                        <i class="fa fa-pencil-square-o"></i></span></div>
                            </div><br>
                            <label for="">Nama Dokter & Perawat & Bidan</label>
        ...
        <script type="text/javascript">
            $(document).on("click", "#modelkonsultasi", function() {
                var Id = $(this).data('id');
                var Konsultasi = $(this).data('namakonsultasi');
                var Iddokter = $(this).data('iddokter');

                $(".updatekonsultasi #idkonsultasi").val(Id);
                $(".updatekonsultasi #konsultasi").val(Konsultasi);
                $(".updatekonsultasi #namadokter").val(Iddokter);
            })
            $(document).ready(function(e) {
                $("#form").on("submit", (function(e) {
                    e.preventDefault();
                    $.ajax({
                        url: '<?= base_url('dokterperawatbidan/ubahkonsultasi'); ?>',
                        type: 'POST',
                        ...
                        processData: false,
                        success: function() {
                            document.location.href = "<?= base_url('dokterperawatbidan/konsultasi'); ?>";
                        }
                    });
                }));
            })
